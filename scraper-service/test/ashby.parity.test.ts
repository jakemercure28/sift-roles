/**
 * Golden: the native TS ashby scraper must emit the committed JobLeads in
 * fixtures/ashby/expected.json. It runs against the same committed fixtures and
...
const fixtures = resolve(here, 'fixtures', 'ashby');
...
test('ashby: native TS output matches committed golden', async () => {
  ...
  try {
    const { scrapeAshby: tsScrape } = await import('../src/scrapers/ashby.js');
